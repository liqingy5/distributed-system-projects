...
def get_args(argv):
    parser = argparse.ArgumentParser(
        description="Script for testing CS 2510 Project 2")
    subparsers = parser.add_subparsers(
        required=True, help='available commands')

    parser_init = subparsers.add_parser(
        'init', description='Initialize Docker bridge network and containers')
    parser_init.set_defaults(func=init_all)

    parser_rm = subparsers.add_parser(
        'rm', description='Remove Docker bridge network and containers')
    parser_rm.set_defaults(func=cleanup)

    parser_loss = subparsers.add_parser(
        'loss', description='Set loss rate between two containers')
    parser_loss.add_argument('container1', type=int,
                             help='first container (e.g. 1 for server id 1)')
    parser_loss.add_argument('container2', type=int,
                             help='second container (e.g. 2 for server id 2)')
    parser_loss.add_argument(
        'loss_rate', type=float, help='loss rate to set (e.g. 10.5 adds 10.5%%  loss)')
    parser_loss.set_defaults(func=set_loss)

    parser_part = subparsers.add_parser(
        'partition', description='Create network partitions')
    ...
    parser_kill = subparsers.add_parser(
        'kill', description='Kill a specific server')
    ...
    parser_relaunch = subparsers.add_parser(
        'relaunch', description='Re-launch a specific server that was previously killed')
    parser_relaunch.add_argument(
        'id', type=int, help='ID of server to relaunch')
...
def init_network():
    cmd_str = "docker network create --driver bridge --subnet {sub} --gateway {gate} {net}".format(
        sub=IP_BASE+'0/24', gate=IP_BASE+'1', net=NETWORK_NAME)
...
def _init_netem(container_name):
    # Set up base class
    cmd_str = "docker exec {} tc qdisc add dev eth0 root handle 1: htb default 10".format(
        container_name)
    subprocess.run(cmd_str, shell=True)
    cmd_str = "docker exec {} tc class add dev eth0 parent 1: classid 1:1 htb rate 100Mbps".format(
        container_name)
    subprocess.run(cmd_str, shell=True)
    cmd_str = "docker exec {} tc class add dev eth0 parent 1:1 classid 1:10 htb rate 100Mbps".format(
        container_name)
    subprocess.run(cmd_str, shell=True)
    cmd_str = "docker exec {} tc qdisc add dev eth0 parent 1:10 handle 10: netem limit 100000 delay 0ms".format(
        container_name)
    subprocess.run(cmd_str, shell=True)

    # Set up special class per server
    for j in range(1, NUM_SERVERS+1):
        cmd_str = "docker exec {} tc class add dev eth0 parent 1:1 classid 1:{} htb rate 100Mbps".format(
            container_name, 10+j)
        subprocess.run(cmd_str, shell=True)
    for j in range(1, NUM_SERVERS+1):
        cmd_str = "docker exec {} tc qdisc add dev eth0 parent 1:{} handle {}: netem limit 1000 delay 0ms".format(
            container_name, 10+j, 10+j)
        # cmd_str = "docker exec {} tc qdisc add dev eth0 parent 1:{} handle {}: netem limit 100000 delay 0ms".format(container_name, 10+j, 10+j)
        subprocess.run(cmd_str, shell=True)
    for j in range(1, NUM_SERVERS+1):
        cmd_str = "docker exec {} tc filter add dev eth0 protocol ip parent 1:0 prio 1 u32 match ip dst {} flowid 1:{}".format(
            container_name, server_ip(j), 10+j)
        subprocess.run(cmd_str, shell=True)
...
def _set_loss(id1, id2, loss):
    ...
    cmd_str1 = "docker exec {} tc qdisc change dev eth0 parent 1:{} handle {}: netem loss {}".format(
        s1, id2+10, id2+10, loss)
    # Set loss outgoing from container2 to container1
    cmd_str2 = "docker exec {} tc qdisc change dev eth0 parent 1:{} handle {}: netem loss {}".format(
        s2, id1+10, id1+10, loss)
